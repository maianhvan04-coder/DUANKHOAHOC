...
function StudentRouteFallback({ text }: { text: string }) {
  return (
    <main className="min-h-[520px] bg-slate-50 px-4 py-16 md:px-6 dark:bg-slate-950">
      <div className="mx-auto flex max-w-3xl items-center justify-center rounded-3xl border border-slate-200 bg-white px-6 py-16 text-center shadow-sm dark:border-white/10 dark:bg-slate-900">
...
export default function StudentGuard({ children }: { children: ReactNode }) {
  ...
  if (!ready) {
    return <StudentRouteFallback text={t("guard.checking")} />;
  }

  if (!user) {
    return <StudentRouteFallback text={t("guard.redirecting")} />;
  }
